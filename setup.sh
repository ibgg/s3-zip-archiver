...
aws lambda create-function \
	--function-name arn:aws:lambda:$REGION:$USERID:function:$ARCHIVER_FUNCTION_NAME \
	--zip-file fileb://$ARCHIVER_FUNCTION_NAME/function.zip \
...
aws lambda update-function-configuration \
	...
	--environment "Variables={BUCKET=$BUCKET_NAME, REGION=$REGION}" \
	--timeout $TIMEOUT \
...
echo "Creating SNS topic for lambda errors"
aws sns create-topic \
    --name $ERRORS_ARCHIVER_SNS_NAME

aws lambda update-function-configuration
	--function-name $DISPATCHER_FUNCTION_NAME \
	--dead-letter-config TargetArn=arn:aws:sns:$REGION:$USERID:$ERRORS_ARCHIVER_SNS_NAME
...
aws apigateway create-rest-api \
	...
	--description "This API archives files in s3"
